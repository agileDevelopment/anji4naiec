...
public class Evolver implements Configurable {
    ...
    private static final Logger logger = Logger.getLogger(Evolver.class.getName());
    /**
    ...
    private static final String PRESENTATION_ACTIVE_KEY = "presentation.active";
    /**
    ...
    public void init(Properties props) throws Exception {
        ...
        // presentation
        if (props.getBooleanProperty(PRESENTATION_ACTIVE_KEY, true)) {
        ...
        // fitness function
        EvaluationFunction fitnessFunc = (EvaluationFunction) props.singletonObjectProperty(FITNESS_FUNCTION_CLASS_KEY);
        config.setEvaluationFunction(fitnessFunc);
    ...
    public void run() throws Exception {
        ...
        for (int generation = 0; (generation < numEvolutions && adjustedFitness < targetFitness); ++generation) {
            ...
            // result data
            champ = genotype.getFittestChromosome();
            adjustedFitness = (maxFitness > 0 ? (double) champ.getFitnessValue() / maxFitness : champ.getFitnessValue());
    ...
    public static void main(String[] args) throws Throwable {
        try {
            ...
            if (propertiesFile.exists()) {
                logger.info("Located the properties file: " + args[0]);            
            } else {
                logger.warning("Could not locate the properties file: " + args[0]);
            }
