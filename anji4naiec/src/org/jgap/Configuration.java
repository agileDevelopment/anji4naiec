...
public class Configuration implements java.io.Serializable {
     ...
     */
    private EvaluationFunction m_bulkObjectiveFunction = null;
    /**
     ...
     */
    private FitnessSelector m_fitnessSelector = null;
    /**
     * References the NaturalSelector implementation that will be used to determine which
     * chromosomes are chosen to be a part of the next generation population.
     */
    private NoveltySelector m_noveltySelector = null;
     ...
     */
    public synchronized void setFitnessSelector(FitnessSelector selector)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        m_fitnessSelector = selector;
    }
     ...
     * @throws InvalidConfigurationException
     */
    public synchronized void setNoveltySelector(NoveltySelector selector)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        m_noveltySelector = selector;
    }

    /**
     * @return selector
     */
    public NoveltySelector getNoveltySelector() {
        return m_noveltySelector;
    }
     ...
     */
    private List<FitnessReproductionOperator> fitnessReproductionOperators = new ArrayList<FitnessReproductionOperator>();
    /**
     * Stores all of the ReproductionOperator implementations that are to be used to operate upon
     * the chromosomes of a population after natural selection. In general, operators will be
     * executed in the order that they are added to this list. Added by Tucker and James.
     */
    private List<NoveltyReproductionOperator> noveltyReproductionOperators = new ArrayList<NoveltyReproductionOperator>();
    /**
     ...
     */
    public synchronized void addFitnessReproductionOperator(FitnessReproductionOperator a_operatorToAdd)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        ...
        fitnessReproductionOperators.add(a_operatorToAdd);
    }
     ...
     */
    public List<FitnessReproductionOperator> getFitnessReproductionOperators() {
        return fitnessReproductionOperators;
    }
     ...
     * this object is locked.
     */
    public synchronized void addNoveltyReproductionOperator(NoveltyReproductionOperator a_operatorToAdd)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        if (a_operatorToAdd == null) {
            throw new InvalidConfigurationException(
        ...
        noveltyReproductionOperators.add(a_operatorToAdd);
    }
     ...
     * @return The list of reproduction operators added to this Configuration
     */
    public List<NoveltyReproductionOperator> getNoveltyReproductionOperators() {
        return noveltyReproductionOperators;
    }
     ...
     */
    private int m_iecPopulationSize = 0;
    private int m_noveltyPopulationSize = 0;
    private int m_fitnessPopulationSize = 0;
    /**
     ...
     */
    public synchronized void setEvaluationFunction(EvaluationFunction a_functionToSet)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
     ...
     */
    public EvaluationFunction getBulkFitnessFunction() {
     ...
     * is locked.
     */
    public synchronized void setIECpopulationSize(int a_sizeOfPopulation)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        ...
        m_iecPopulationSize = a_sizeOfPopulation;
    }
    ...
    public synchronized void lockSettings() throws InvalidConfigurationException {
        if (!m_settingsLocked) {
            ...
            // --------------------------------------
            fitnessReproductionOperators = Collections.unmodifiableList(fitnessReproductionOperators);
    ...
    public synchronized void verifyStateIsValid() throws InvalidConfigurationException {
        ...
        if (fitnessReproductionOperators.isEmpty()) {
            throw new InvalidConfigurationException(
        ...
        float totalSlices = getFitnessSelector().getSurvivalRate();
        Iterator it = getFitnessReproductionOperators().iterator();
        while (it.hasNext()) {
            FitnessReproductionOperator oper = (FitnessReproductionOperator) it.next();
    ...
    private int m_evolutionMode = GENERATIONAL;

    public void setEvolutionMode(int modeType) {
        m_evolutionMode = modeType;
    }

    public boolean isGenerational() {
        if (m_evolutionMode == GENERATIONAL) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean isSteadyState() {
        if (m_evolutionMode == STEADY_STATE) {
            return true;
        } else {
            return false;
        }
    }
    public static final int GENERATIONAL = 0;
    public static final int STEADY_STATE = 1;

    private int m_selectionMode = FITNESS;

    public void setSelectionMode(int modeType) {
        if (modeType == FITNESS) {
            m_selectionMode = FITNESS;
        } else if (modeType == NOVELTY) {
            m_selectionMode = NOVELTY;
        }
    }

    public boolean selectForFitness() {
        return m_selectionMode == FITNESS;
    }

    public boolean selectForNovelty() {
        return m_selectionMode == NOVELTY;
    }
    
    public static final int FITNESS = 0;
    public static final int NOVELTY = 1;

    /**
     * Sets the population size to be used for this genetic algorithm when performing novelty search. 
     * The population size is a fixed value that represents the number of Chromosomes contained within 
     * the Genotype (population). This setting is required.
     * 
     * @param a_sizeOfPopulation The population size to be used.
     * 
     * @throws InvalidConfigurationException if the population size is not positive or this object
     * is locked.
     */
    public synchronized void setNoveltySearchPopulationSize(int a_sizeOfPopulation)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        // Sanity check: Make sure the population size is positive.
        // --------------------------------------------------------
        ...
        m_noveltyPopulationSize = a_sizeOfPopulation;
    }

    /**
     * Retrieves the novelty search population size setup in this Configuration instance.
     * 
     * @return The novelty search population size.
     */
    public int getNoveltySearchPopulationSize() {
        return m_noveltyPopulationSize;
    }

    private int m_shortTermNoveltySearchLimit = 500;
    public void setShortTermNoveltySearchLimit(int an_evaluationLimit) throws InvalidConfigurationException {
        verifyChangesAllowed();
        // Sanity check: Make sure the evaluation limit is positive.
        // --------------------------------------------------------
        if (an_evaluationLimit < 1) {
            throw new InvalidConfigurationException("The short-term novelty search limit must be positive.");
        }
        m_shortTermNoveltySearchLimit = an_evaluationLimit;
    }
    
    public int getShortTermNoveltySearchLimit() {
        return m_shortTermNoveltySearchLimit;
    }
     ...
     * is locked.
     */
    public synchronized void setFitnessSearchPopulationSize(int a_sizeOfPopulation) throws InvalidConfigurationException {
        verifyChangesAllowed();
        // Sanity check: Make sure the population size is positive.
        // --------------------------------------------------------
        if (a_sizeOfPopulation < 1) {
        ...
        m_fitnessPopulationSize = a_sizeOfPopulation;
    }
     ...
     * @return The novelty search population size.
     */
    public int getFitnessSearchPopulationSize() {
        return m_fitnessPopulationSize;
    }    

    private int m_shortTermFitnessSearchLimit = 500;
    public void setFitnessSearchLimit(int an_evaluationLimit) throws InvalidConfigurationException {
        verifyChangesAllowed();
        // Sanity check: Make sure the evaluation limit is positive.
        // --------------------------------------------------------
        if (an_evaluationLimit < 1) {
            throw new InvalidConfigurationException("The short-term fitness search limit must be positive.");
        }
        System.out.println("Fitness search limit changed to " + an_evaluationLimit + " evaluations.");
        m_shortTermFitnessSearchLimit = an_evaluationLimit;
    }
    
    int getShortTermFitnessSearchLimit() {
        return m_shortTermFitnessSearchLimit;
    }
}
