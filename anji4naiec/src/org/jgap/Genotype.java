...
public class Genotype extends ProgressSource implements Serializable {
    ...
    private static final Logger logger = Logger.getLogger(Genotype.class.getName());
    /**
...
//        fitnessMutationPower = Math.max(aMutationPower, 0.0f);
     ...
     */
    public Genotype(Configuration a_activeConfiguration, List<ChromosomeMaterial> a_initialChromosomes)
     ...
     */
    private void addChromosomesFromMaterial(Collection<ChromosomeMaterial> chromosomeMaterial) {
...
//            return new ArrayList<Chromosome>(m_chromosomes);
    ...
    public synchronized void evolveForFitness() {
        ...
        try {
            ...
            if (m_activeConfiguration.isSteadyState()) {
                ...
                adjustChromosomeList(offspring, m_activeConfiguration.getFitnessSearchPopulationSize() - m_chromosomes.size());
            ...
            Logger.getLogger(Genotype.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            writeLock.unlock();
        }
    }
    ...
    public synchronized static Genotype novelInitialGenotype(Configuration a_activeConfiguration) throws InvalidConfigurationException {
...
//            Collections.sort(m_chromosomes);
        ...
        Collections.sort(genotype.m_chromosomes, new ChromosomeNoveltyComparator(false, false));
     ...
     */
    public synchronized static Genotype randomInitialGenotype(Configuration a_activeConfiguration)
            throws InvalidConfigurationException {
        if (a_activeConfiguration == null) {
            throw new IllegalArgumentException("The Configuration instance may not be null.");
        }
    ...
    public synchronized static Genotype emptyInitialGenotype(Configuration a_activeConfiguration)
            throws InvalidConfigurationException {
        if (a_activeConfiguration == null) {
            throw new IllegalArgumentException("The Configuration instance may not be null.");
        }
        a_activeConfiguration.lockSettings();
        return new Genotype(a_activeConfiguration, new ArrayList<ChromosomeMaterial>());
    }
    ...
    private void evaluateForNovelty() {
        ...
        // -------------------------------------
        evaluateForFitness();
        m_activeConfiguration.getBulkFitnessFunction().evaluateNovelty(m_chromosomes);
    }

    private void evaluateForFitness() {
        // Evaluate the new population of of chromosomes.
        // -------------------------------------
        m_activeConfiguration.getBulkFitnessFunction().evaluateFitness(m_chromosomes);
    }
    ...
    private void expandPopulation(int targetPopulationSize) {
        ...
        try {
            ...
            // Execute Mutation Operators.
            // -------------------------------------
            adjustChromosomeList(offspring, targetPopulationSize - m_chromosomes.size());
...
//    }
    public synchronized int getCurrentNoveltyArchiveSize() {
        readLock.lock();
        try {
            return m_activeConfiguration.getBulkFitnessFunction().getNoveltyArchiveSize();
        } finally {
            readLock.unlock();
        }
    }
    ...
    public synchronized double getCurrentNoveltyThreshold() {
        readLock.lock();
        try {
            return m_activeConfiguration.getBulkFitnessFunction().getNoveltyThreshold();
        } finally {
            readLock.unlock();
        }
    }

    public synchronized Map<Chromosome, BehaviorVector> getAllPointsVisited() {
        readLock.lock();
        try {
            return m_activeConfiguration.getBulkFitnessFunction().getAllPointsVisited();
        } finally {
            readLock.unlock();
        }
    }
    ...
    private void noveltyTask() {
        ...
        for (int j = i; j < m_activeConfiguration.getShortTermNoveltySearchLimit(); j++) {
            // Evaluate the new population of of chromosomes.
            // -------------------------------------
            for (Chromosome chrom : m_chromosomes) {
            ...
            updateProgressListeners(noveltyProgress(novelChromosomes.size(), j), chromosomeIdArchive.size());
            replaceLeastNovelIndividual();
        }
        ...
        // -------------------------------------
        Collections.sort(m_chromosomes, new ChromosomeNoveltyComparator(false, false));
    ...
    private void optimizationTask() {
        ...
        // Mark the fittest user slected chromosome as Protected
        // -------------------------------------
        Chromosome seed = fittestChromosome();
        seed.protect();
        ...
        // -------------------------------------
        for (i = i; i < m_activeConfiguration.getShortTermFitnessSearchLimit(); i++) {
            updateProgressListeners(fitnessProgress(i), chromosomeIdArchive.size());
