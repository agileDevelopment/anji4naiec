 ...
 * guaranteed.
 */
public class WeightedNoveltySelector extends NoveltySelector {
    ...
    protected synchronized void add(Configuration a_activeConfigurator,
        ...
        // combined total of their novelty values). If the Chromosome is
        // already in the Map, then we just increment its number of slots
        // by its novelty value. Otherwise we add it to the Map.
        // -----------------------------------------------------------------
        ...
        if (counter != null) {
            ...
            counter.reset(a_chromosomeToAdd.getSpeciatedNoveltyValue());
            m_wheel.put(a_chromosomeToAdd, counter);
        }

        m_totalNumberOfUsedSlots += a_chromosomeToAdd.getSpeciatedNoveltyValue();
    }
     ...
     */
    protected synchronized List<Chromosome> select(Configuration a_activeConfiguration, int a_howManyToSelect) {
        ...
        if (m_wheel.isEmpty() == false) {
            ...
            // that contains the novelty values for each chromosome, one that
            ...
            // together (eg, if a chromosome is at index 5, then its novelty value
            // and counter values are also at index 5 of their respective arrays).
            // -------------------------------------------------------------------
            Set entries = m_wheel.entrySet();
            int numberOfEntries = entries.size();
            int[] noveltyValues = new int[numberOfEntries];
            ...
            for (int i = 0; i < numberOfEntries; i++) {
                ...
                noveltyValues[ i] = currentCounter.getNoveltyValue();
            ...
            while (result.size() < a_howManyToSelect) {
                Chromosome selectedChromosome = spinWheel(generator, noveltyValues, counterValues,
                        chromosomes);
     ...
     * @param a_noveltyValues
     * @param a_counterValues
     * @param a_chromosomes
     * @return selected chromosome
     */
    private Chromosome spinWheel(Random a_generator, int[] a_noveltyValues,
        ...
        // how this works: we have three arrays, one with the novelty values
        ...
        // then its novelty value and counter value are also at index 5 of
        ...
        for (int i = 0; i < a_counterValues.length; i++) {
            ...
            if (currentSlot > selectedSlot) {
                ...
                // decrementing the slot counter by the novelty value.
                // --------------------------------------------------------
                a_counterValues[ i] -= a_noveltyValues[ i];
                m_totalNumberOfUsedSlots -= a_noveltyValues[ i];
    ...
    private class SlotCounter {
         ...
         */
        public void incrementByNovelty() {
