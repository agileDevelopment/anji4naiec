...
import org.jgap.impl.WeightedFitnessSelector;

import org.jgap.impl.SimpleFitnessSelector;
 ...
 */
public abstract class FitnessReproductionOperator implements ReproductionOperatorInterface {
    ...
    final public void reproduce(final Configuration config, final List<Specie> parentSpecies, List<ChromosomeMaterial> offspring)
        ...
        if (config.isSteadyState()) {
            if (offspring.size() < 1) {
                ...
                if (config.getFitnessSelector() instanceof WeightedFitnessSelector) {
                    ...
                    for (Specie currSpecie : parentSpecies) {
                        ladder += currSpecie.getFitnessValue() / totalSpeciesFitness;
                        if (ladder >= threshold) {
                            parentSpecie = currSpecie;
                        }
                    }
                }
                if (config.getFitnessSelector() instanceof SimpleFitnessSelector) {
                    for (Specie currSpecie : parentSpecies) {
                        if (parentSpecie == null) {
                            parentSpecie = currSpecie;
                        }
                        if (currSpecie.getFitnessValue() > parentSpecie.getFitnessValue()) {
                            parentSpecie = currSpecie;
                        }
                    }
                }
                // Select a species based on the average fitness
